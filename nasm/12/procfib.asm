...
.DATA
prompt      db      "Please input a positive number (>1): ", 0
...
.CODE
    .STARTUP
    PutStr      prompt              ; request input number
...
;-------------------------------------------------------------
; Procedure Fibonacci receivers an integer in EDX and computes
; the largest Fibonacci number that is less than the input number.
; The Fibonacci number is returned in EAX
;-------------------------------------------------------------
...
fibonacci:
    ...
    ; numbers computed;
    ; FIB_HI miantains the larger one.
    mov         FIB_LO, 1   ; initialize FIB_ZO and FIB_HI
    mov         FIB_HI, 1   ; to first two Fibonacci numbers
fib_loop:
