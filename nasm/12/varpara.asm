...
;   Input:  Request variable number of nonzero integers.
...
read_number:
    ...
    push        EAX                 ; place teh number on stack
    inc         ECX                 ; increment number count
    jmp         read_number
stop_reading:
    push        ECX                 ; place number count on stack
    call        variable_sum        ; return sum in EAX
    ...
    add         ESP,ECX             ; update ESP to clear parameter
                                    ; space on the stack
    PutStr      sum_msg             ; dispaly the sum
    PutLInt     EAX
    nwln
done:
    .EXIT
...
;----------------------------------------------------------------
; This procedure receives variable number of integers via the stack
; The last paramenter pushed on the stack should be the number of
...
variable_sum:
    ...
    mov         ECX, [EBP+8]    ; ECX = $ of integersto be add
    mov         EBX, EBP
    add         EBX, 12         ; EBX = pointer to first number
    sub         EAX, EAX        ; sum = 0
add_loop:
    add         EAX, [SS:EBX]   ; sum = sum + next number
    add         EBX,4           ; EBX points to the next number
